...
#include "commands/needs-restarting.h"
...
namespace
{
  ...
  static std::map< ZypperCommand::Command, CommandFactory > &newStyleCommands ()
  {
    static std::map< ZypperCommand::Command,  CommandFactory> table {
      ...
      { ZypperCommand::NEEDS_RESTARTING_e, CommandFactory::make<NeedsRestartingCmd>( { "needs-restarting" }) }
    };
    return table;
  }
...
DEF_ZYPPER_COMMAND( NEEDS_RESTARTING );
